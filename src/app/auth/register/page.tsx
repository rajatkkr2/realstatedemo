...
export default function RegisterPage() {
  ...
  const roles = [
    { value: "buyer", label: "User / Buyer", desc: "Browse and inquire about properties", color: "0,240,255" },
    { value: "admin", label: "Admin", desc: "Manage listings, inquiries & users", color: "255,0,170" },
  ];
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <GlassCard className="p-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your Name"
                    className="w-full rounded-xl bg-white/5 py-3 pl-10 pr-4 text-sm text-white placeholder-white/30 outline-none ring-1 ring-white/10 focus:ring-[var(--neon-purple)]/30" required />
                </div>
              </div>
              <div>
              ...
              <div>
                <label className="mb-1.5 block text-[10px] font-medium tracking-wider text-white/40">PHONE</label>
                <div className="relative">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 6 characters"
                    className="w-full rounded-xl bg-white/5 py-3 pl-10 pr-4 text-sm text-white placeholder-white/30 outline-none ring-1 ring-white/10 focus:ring-[var(--neon-purple)]/30" required minLength={6} />
                </div>
              </div>
              <div>
                <label className="mb-1.5 block text-[10px] font-medium tracking-wider text-white/40">SELECT ROLE *</label>
                <div className="grid grid-cols-2 gap-2">
                  {roles.map((r) => (
                    <button key={r.value} type="button" onClick={() => setRole(r.value)}
                      className={`rounded-xl px-3 py-3 text-left transition-all ${role === r.value ? "text-white" : "text-white/40 hover:bg-white/5"}`}
                      style={role === r.value ? { background: `rgba(${r.color},0.12)`, border: `1px solid rgba(${r.color},0.3)` } : { border: "1px solid rgba(255,255,255,0.08)" }}>
                      <div className="flex items-center gap-2 mb-0.5">
                        {r.value === "admin" ? <Shield size={14} /> : <User size={14} />}
              ...
              <NeonButton type="submit" variant="purple" className="w-full" disabled={isLoading}>
